...
def site_digest():
    return hashlib.sha256(
        json.dumps(site_manifest(), sort_keys=True, separators=(',', ':')).encode()
    ).hexdigest()
...
if actual != EXPECTED:
    errors.append('Hero source SHA-256 changed')
if '--decode' in sys.argv:
    from PIL import Image
    with Image.open(image) as im:
        im.load()
        if im.size != (864, 1536) or im.format != 'JPEG':
            errors.append('Incorrect hero dimensions/format')

for name in ('index.html', 'framework.html'):
    text = (ROOT / name).read_text(encoding='utf-8')
    import re
    pattern = (
        r'<h3>(Stabilize|Observe|Separate|Choose|Execute|Measure|Document|Adjust)</h3>'
        if name == 'index.html'
        else r'\d\d / (Stabilize|Observe|Separate|Choose|Execute|Measure|Document|Adjust)'
    )
    if re.findall(pattern, text) != [
        'Stabilize', 'Observe', 'Separate', 'Choose',
        'Execute', 'Measure', 'Document', 'Adjust'
    ]:
        errors.append(f'{name}: method order changed')

text = (ROOT / 'index.html').read_text(encoding='utf-8')
for required in (
    '<h1 id="hero-title">EXIT FRAMEWORK</h1>',
    ...
    if required not in text:
        errors.append(f'Homepage missing {required}')

css = (ROOT / 'styles.css').read_text(encoding='utf-8')
js = (ROOT / 'site.js').read_text(encoding='utf-8')
if '!important' in css or 'background-image' in css or 'data:image' in css:
    errors.append('Competing style/hero logic found')
if 'createElement' in js or 'fetch(' in js:
    errors.append('Unexpected injection/integration logic')
if (ROOT / 'CNAME').read_text().strip() != 'chaosexit.com':
    errors.append('Domain mismatch')

if '--release' in sys.argv:
    report = json.loads((ROOT / 'qa-results.json').read_text())
    if report.get('result') != 'passed' or report.get('widths') != [390, 430, 768, 1440]:
        errors.append('Mandatory browser QA has not passed')
    if report.get('image_sha256') != EXPECTED:
        errors.append('Browser QA references different artwork')
    if report.get('site_sha256') != site_digest():
        expected = report.get('site_manifest', {})
        actual_manifest = site_manifest()
        changed = [
            name for name in sorted(set(expected) | set(actual_manifest))
            if expected.get(name) != actual_manifest.get(name)
        ]
